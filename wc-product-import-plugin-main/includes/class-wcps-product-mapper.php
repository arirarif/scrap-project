...
class WCPS_Product_Mapper {
    ...
    public function map_product($json_product, $existing_product_id = null) {
        try {
            ...
            // Basic fields
            $product->set_name($json_product['Name']);
            $product->set_sku($this->generate_sku($json_product['Id']));
            $product->set_regular_price((string) $json_product['Price']);
            $product->set_manage_stock(true);
            $product->set_stock_quantity((int) ($json_product['Stock'] ?? 0));
            $product->set_stock_status($json_product['Stock'] > 0 ? 'instock' : 'outofstock');
     ...
     * @return string
     */
    private function generate_sku($external_id) {
        return 'SYNC-' . $external_id;
    }
     ...
     */
    private function create_attribute($name, $values) {
     ...
     */
    private function get_or_create_global_attribute($label, $slug) {
        // Check cache
    ...
    public function link_alternative_products() {
        ...
        foreach ($products as $row) {
            ...
            foreach ($linked_external_ids as $external_id) {
                $linked_id = $this->find_product_by_external_id($external_id);
